$(document).ready(function () {
    ...
    //Map Accord
    $(document).on('click', '.p-map-hide', function(){

        if($('.location-pins').is(':visible')){
            $(this).text("Show Map");
        }
        else{
            $(this).text("Hide Map");
    ...
    //FAQ Accord
    $(document).on('click', '.p-faq-ques', function(){
        $(this).toggleClass('active').next().slideToggle();
    });

    //Tab Function
    $(document).on("click", "[data-tab] li", function(e) {
            e.preventDefault();
            var getDataVal = $(this).parent("[data-tab]").attr("data-tab");
            $(this).addClass("active").siblings().removeClass("active");
            var getInd = $(this).index();
            $("[data-tabdata='" + getDataVal + "'] > * ").eq(getInd).show().siblings().hide();
    });

    //Confirm Identiy SMS code
    $(document).on('click', '.p-sms-txt', function(e){
        e.preventDefault();
        $('.p-sms-inp').show();
        $('.p-upper-txt').text('Haven’t received your code?');
        $('.p-lower-txt').text('This could take a few mintues to process. Be sure the mobile number you entered is correct. If not, update and re-send code.');
    });

    //Responsive accordian
    $(document).on('click', '.p-res-accord', function(){
        $(this).toggleClass('active').next().slideToggle();
    });

    //Filter Toggle
    $(document).on('click', '.p-filter-toggle', function(){
        $('.filters').toggleClass('active');
        $('body').toggleClass('scroll-stop-alt p-overlay-alt');
    });

    $(document).on('click', '.p-filter-close', function(){
        $(this).parents('.filters').removeClass('active');
        $('body').removeClass('scroll-stop-alt p-overlay-alt');
    });

    //ASAP Toggle
    $(document).on('click', '.p-asap-toggle', function(){
        
        if(!($(this).hasClass('active'))) {
            $(this).addClass('active').children('.p-asap-wrap').toggle();
        }

    });
    $(document).on('click', function(e){
        if(!$(e.target).is('.p-asap-toggle , .p-asap-toggle *')){
            $('.p-asap-wrap').hide();
            $('.p-asap-toggle').removeClass('active');
        }
    });
...
//    });
  ...
  /*footer*/
    ...
    var halt = $('.p-filter-wrap').outerHeight();
    ...
    if (($(window).width() >= 768) && ($(window).width() <= 1000)) {
        $('.filters').height(window.innerHeight - hh - halt);
    }
    else if ($(window).width() < 768) {
        $('.filters').height(window.innerHeight - hh);
    }
